...
cons = cons.loc[:,'population':'other.movers'].astype('int64')

num_da = len(cons)


for i in range(0,num_da):
    ...
    for j in range (0, population_num):
        ...
        total = cons['no.married'][i] + cons['married'][i]+cons['living.common.law'][i]+cons['separated'][i]+cons['divorced'][i]+cons['widowed'][i]
        if (total==0):
            print(i)
            print("marst")
        
        per_nm = cons['no.married'][i] / total * 100
        per_m = cons['married'][i] / total * 100
        per_lcl = cons['living.common.law'][i] / total * 100
        ...
        chance_cat3 = chance_cat2 + per_lcl;
        chance_cat4 = chance_cat3 + per_s;
        chance_cat5 = chance_cat4 + per_d;
        
        if (dice_roll < chance_cat1):
            marst = 'no.married'
        elif (dice_roll >= chance_cat1 and dice_roll <= chance_cat2):
            marst = 'married'
        elif (dice_roll > chance_cat2 and dice_roll <= chance_cat3):
            marst = 'living.common.law'
        elif (dice_roll > chance_cat3 and dice_roll <= chance_cat4):
            marst = 'separated'
        elif (dice_roll > chance_cat4 and dice_roll <= chance_cat5):
            marst = 'divorced'
        else:
            marst = 'widowed'


        total = cons['single.detached.house'][i] + cons['apartment'][i]+cons['other.attached'][i]
        if (total==0):
            print(i)
            print("dwel")
        
        per_sdh = cons['single.detached.house'][i] / total * 100
        per_a = cons['apartment'][i] / total * 100
        per_oa = cons['other.attached'][i] / total * 100
        
        random.seed((i+1)*(j+1)*7)
        dice_roll = random.random()*100
        
        chance_cat1 = per_sdh;
        chance_cat2 = chance_cat1 + per_a;

        
        if (dice_roll < chance_cat1):
            dwel = 'single.detached.house'
        elif (dice_roll >= chance_cat1 and dice_roll <= chance_cat2):
            dwel = 'apartment'
        else:
            dwel = 'other.attached'
        ...
        total = cons['non.movers'][i] + cons['nonmigrants.movers'][i]+cons['interprovincial.movers'][i]+cons['external.movers'][i]+cons['other.movers'][i]
        if (total==0):
            print(i)
            print("mobl")
        
        per_m = cons['non.movers'][i] / total * 100
        per_nmm = cons['nonmigrants.movers'][i] / total * 100
        per_ipm = cons['interprovincial.movers'][i] / total * 100
        per_em = cons['external.movers'][i] / total * 100
        per_om = cons['other.movers'][i] / total * 100
        
        random.seed((i+1)*(j+1)*7)
        dice_roll = random.random()*100
        
        chance_cat1 = per_m;
        chance_cat2 = chance_cat1 + per_nmm;
        chance_cat3 = chance_cat2 + per_ipm;
        chance_cat4 = chance_cat3 + per_em;
        
        if (dice_roll < chance_cat1):
            mobl = 'non.movers'
        elif (dice_roll >= chance_cat1 and dice_roll <= chance_cat2):
            mobl = 'nonmigrants.movers'
        elif (dice_roll > chance_cat2 and dice_roll <= chance_cat3):
            mobl = 'interprovincial.movers'
        elif (dice_roll > chance_cat3 and dice_roll <= chance_cat4):
            mobl = 'external.movers'
        else:
            mobl = 'other.movers'



        ind=['NA', 933, sex, age_group, ethnic_origin, official_lang, work_official_lang, marst, dwel, mobl]
        List.append(ind)
        
    df = pd.DataFrame(List)
    df.columns =['PPSORT', 'CMA', 'AGE', 'SEX', 'RACE', 'KOL', 'WORKLANG','MARST','DWEL','MOBL']
    df['PPSORT'] = inds['PPSORT']
    #df.to_csv('Unmerged_data/GeneratedPop/Vancouver2/{num}.csv'.format(num=str(i+1)),index=False)
    df.to_csv('../synthetic_data/Canada/DA_ADD/conditional_probability/{num}.csv'.format(num=str(i+1)),index=False)
    #print(df)