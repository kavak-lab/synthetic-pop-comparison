...
def goodness_fit(sol, sample, constraints):
    ...
    synth_block_group = [len(df), dc['a0_4'], dc['a5_9'],
                         ...
                         dc['living.common.law'], dc['separated'], dc['divorced'], dc['widowed'],
                         dc['single.detached.house'],
                         dc['apartment'], dc['other.attached'], dc['non.movers'], dc['nonmigrants.movers'],
                         dc['interprovincial.movers'],
...
def hill_climbing(i, minute_limit):
    ...
    tempCons = cons.loc[[i], 'population':'other.movers']  # added to resolve the error
...
def generate_population(i):
    ...
    ans.to_csv('../synthetic_data/Canada/DA_ADD/hill_climbing/{num}.csv'.format(num=str(i + 1)), index=False)
