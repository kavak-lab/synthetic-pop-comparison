...
def goodness_fit(sol, sample, constraints):
    df = sample.loc[sol, :]
    dc = Counter(df[['AGE', 'SEX', 'RACE','KOL','WORKLANG','MARST','DWEL','MOBL']].values.flatten())
    ...
    synth_block_group = [len(df), dc['a0_4'], dc['a5_9'],
                         ...
                         dc['ao_esao_Filipino'], dc['ao_oesao'], dc['oo'],dc['official_lang'],dc['no.official_lang'],
                         dc['work.official_lang'],dc['work.no.official_lang'],dc['no.married'], dc['married'],
                         dc['living.common.law'],dc['separated'],dc['divorced'],dc['widowed'],dc['single.detached.house'],
                         dc['apartment'],dc['other.attached'],dc['non.movers'],dc['nonmigrants.movers'],dc['interprovincial.movers'],
                         dc['external.movers'],dc['other.movers']]
...
def compare_fit(prev_agg, new_ind, prev_ind,constraints):
    ...
    prev_agg[prev_ind['KOL']] += 1
    prev_agg[new_ind['KOL']] -= 1
    prev_agg[prev_ind['WORKLANG']] += 1
    prev_agg[new_ind['WORKLANG']] -= 1
    prev_agg[prev_ind['MARST']] += 1
    prev_agg[new_ind['MARST']] -= 1
    prev_agg[prev_ind['DWEL']] += 1
    prev_agg[new_ind['DWEL']] -= 1
    prev_agg[prev_ind['MOBL']] += 1
    prev_agg[new_ind['MOBL']] -= 1
...
# SIMULATED ANNEALING
def simulated_annealing(i, minute_limit):
    temp_change = 3
    sample_num = len(inds)
    ...
    while t_0 >= 0 and fit0[0] > 0:
        ...
        # while random_ind_samp[0] in orig_solution:
        #    random_ind_samp = np.random.randint(0,sample_num,size=1)
        ...
        # if the new fit is better than old by a certain margin --> temp_change*step
        if fit > new_fit + t_0 * temp_change:
            orig_solution[random_ind_sol[0]] = random_ind_samp[0]
            t_0 -= step
            fit = new_fit
            if time.time() - x > 2.75:
                t_0 = 1.99 / temp_change
            if t_0 <= 0: t_0 = 0
        else:
            fit0[1] = saved_agg
...
def generate_population(i):
    ...
    #sys.stdout.flush()
    ans = simulated_annealing(i, 6)
    ans.to_csv('../synthetic_data/Canada/DA_ADD/simulated_annealing/{num}.csv'.format(num=str(i + 1)), index=False)
