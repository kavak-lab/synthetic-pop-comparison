...
kol_list = list('official_lang', 'no.official_lang')
wol_list = list('work.official_lang', 'work.no.official_lang')
marst_list = list('no.married','married', 'living.common.law', 'separated', 'divorced', 'widowed')
dwel_list = list('single.detached.house', 'apartment', 'other.attached')
mobl_list = list('non.movers', 'nonmigrants.movers', 'interprovincial.movers', 'external.movers', 'other.movers')

total_list_of_names = c(population_list, age_list, gender_list, race_list, kol_list, wol_list, marst_list, dwel_list, mobl_list)
total_number_of_column = length(population_list) + length(age_list) + length(gender_list) + length(race_list) + length(kol_list) + length(wol_list) + length(marst_list) + length(dwel_list) + length(mobl_list)
...
while(block_group_counter<=nrow(cons)){ 
  ...
  kol_column = synth_block_group$KOL
  wol_column = synth_block_group$WORKLANG
  marst_column = synth_block_group$MARST
  dwel_column = synth_block_group$DWEL
  mobl_column = synth_block_group$MOBL
  ...
  while(counter<=population){
    ...
    aggregate_data[kol_column[counter]] = aggregate_data[kol_column[counter]] + 1
    aggregate_data[wol_column[counter]] = aggregate_data[wol_column[counter]] + 1
    aggregate_data[marst_column[counter]] = aggregate_data[marst_column[counter]] + 1
    aggregate_data[dwel_column[counter]] = aggregate_data[dwel_column[counter]] + 1
    aggregate_data[mobl_column[counter]] = aggregate_data[mobl_column[counter]] + 1
...
write.csv(global_aggregate_data, file = paste0("../synthetic_data/Canada/DA_ADD/synthetic_group_data_",abbrev, ".csv"), row.names = FALSE)